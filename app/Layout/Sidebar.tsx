      ...
      <div className="flex h-full flex-col">
        <div className="px-5 py-5">
          <Link href="/dashboard" className="block">
            ...
            <span className="mt-3 block text-lg font-semibold">Bean Counter</span>
            <span className="block text-xs text-[#d8bf9f]">Cafe operations suite</span>
          </Link>
        </div>
          ...
          {navigationItems.map((item) => (
            <div key={item.href} className="shrink-0 lg:shrink">
              <Link
                href={item.href}
                className={cn(
                  "block whitespace-nowrap rounded-2xl px-3 py-2.5 text-sm font-medium text-[#ead8c5] transition hover:bg-white/10 hover:text-white",
                  pathname === item.href &&
